<?php
    ...
    if (file_put_contents($file_name, file_get_contents($url)) && file_put_contents($file_name2, file_get_contents($urlcsv)))
    {
...
$foxarraycsv = $spreadsheetcsv->getActiveSheet()
    ->rangeToArray(
        'K2:N1000',     // The worksheet range that we want to retrieve
        'no_value',        // Value that should be returned for empty cells
        TRUE,        // Should formulas be calculated (the equivalent of getCalculatedValue() for each cell)
        FALSE,        // Should values be formatted (the equivalent of getFormattedValue() for each cell)
        FALSE         // Should the array be indexed by cell row and cell column
    );
...
$foxeditedcsv->getActiveSheet()
    ->fromArray(
        $foxarraycsv,  // The data to set
        'no_value',        // Array values with this value will not be set
        'A2'         // Top left coordinate of the worksheet range where
                     //    we want to set these values (default is A1)
    );
