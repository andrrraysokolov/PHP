...
$activeSheet->setCellValue('B1', 'Quantity');
...
for ($stn = 2; $stn < ($last_rowcsv + 1); $stn++) {
    if (($spreadsheet->getActiveSheet()->getCell('B'.$stn)->getValue()) > 2) {
        $spreadsheet->getActiveSheet()->setCellValue('B'.$stn, '1001');}
}

$writer = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($spreadsheet, "Xlsx");
$writer->save('www/100kwatt.ru/myscripts/rpr_quantity.xlsx');
?>
